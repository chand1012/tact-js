import PlayerSocket, { Message, DEFAULT_RETRY_CONNECT_TIME } from './PlayerSocket'
import {
  DotPoint,
  ErrorCode,
  PathPoint,
...
class HapticPlayer {
  ...
  constructor(url = undefined) {
    if (url) {
      this.socket = new PlayerSocket(DEFAULT_RETRY_CONNECT_TIME, url)
    } else {
  ...
  public submitDot = (
    ...
    return this.socket.send(
      JSON.stringify(request, (k, val) => (val.toFixed ? Number(val.toFixed(3)) : val))
    )
  }
  ...
  public submitRegisteredWithRotationOption = (
    key: string,
